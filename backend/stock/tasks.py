...
mm_url = "https://meeting..com/hooks/71cbfy7fdir6tnops9c7pwscuo"
...
def add_day_stocks():
    ...
    # MM에 스케줄러 시작 알림
    mm_now = dt_now.strftime('%Y-%m-%d')
    start_mm_message = mm_now + ' 에 해당하는 주식 데이터를 불러오기 시작합니다'
    mm_message = {
    ...
    headers = {
        ...
        'User-Agent': 'O2Mc1EmFLez8by7BX0EIh30Rkd32XD+7w/aGiWTvNHQ=' #generate.cmd에서 찾아서 입력
    }
    ...
    for idx, row in df.iterrows():
        ...
        if row['시장구분'] == 'KOSPI':
            kospi_index[0] += int(row['거래량'])
            kospi_index[1] += int(row['거래대금'])
            kospi_index[2] += int(row['시가총액'])
            kospi_index[3] += int(row['상장주식수'])
        elif row['시장구분'] == 'KOSDAQ':
            kosdaq_index[0] += int(row['거래량'])
            kosdaq_index[1] += int(row['거래대금'])
            kosdaq_index[2] += int(row['시가총액'])
            kosdaq_index[3] += int(row['상장주식수'])

        if row['종목코드'] in kospi_code or row['종목코드'] in kosdaq_code or row['종목코드'] in konex_code:
            code_nubmer = row['종목코드']
            current_price = str(int(row['종가']))
            changes = str(row['대비'])
            chages_ratio = str(row['등락률'])
            start_price = str(int(row['시가']))
            high_price = str(int(row['고가']))
            low_price = str(int(row['저가']))
            volume = str(int(row['거래량']))
            trade_price = str(int(row['거래대금']))
            market_cap = str(int(row['시가총액']))
            stock_amount = str(int(row['상장주식수']))
            date = str(t_date)
    ...
    # 코스피 데이터 불러오기
    kospi_day = data.get_data_yahoo("^KS11", t_date, t_date)
    kospi_week = data.get_data_yahoo("^KS11", before_one_week, t_date)
    
    # 코스닥 데이터 불러오기
    kosdaq_day = data.get_data_yahoo("^KQ11", t_date, t_date)
    ...
    for idx, row in kosdaq_day.iterrows():
        ...
        stock_list.append({'code_number' : 'kosdaq', 'current_price' : str(round(row['Close'], 6)), 'changes' : str(round(kosdaq_changes, 2)), 'chages_ratio' : str(kosdaq_change_ratio), 'start_price' : str(round(row['Open'], 6)), 'high_price' : str(round(row['High'], 6)), 'low_price' : str(round(row['Low'], 6)), 'volume' : str(kosdaq_index[0]), 'trade_price' : str(kosdaq_index[1]), 'market_cap' : str(kosdaq_index[2]), 'stock_amount' : str(kosdaq_index[3]), 'date' : str(t_date)})
    
    serializer = DayStockSerializer(data=stock_list, many=True)
    if serializer.is_valid(raise_exception=True):
        serializer.save()
        
    # MM에 스케줄러 종료 알림
    end_time = time.time()
    result_sec = end_time - start_time
    result_time = dt_mm.timedelta(seconds=result_sec)
    
    min = int(result_time.seconds / 60)
    sec = result_time.seconds % 60
    
    total_count = len(stock_list) + 2
    
    end_mm_message = '총 ' + str(total_count) + '개의 주식 데이터가 DB에 저장되었습니다. 소요시간 : ' + str(min) + '분 ' + str(sec) + '초'
